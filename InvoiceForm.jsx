...
export default function InvoiceForm() {
  ...
  const [form, setForm] = useState(
    initState(searchParams.get("type") || "INVOICE"),
  );
        ...
        <div className="form-page-header-left">
          ...
          <div>
            <h1 className="form-page-title">
              {isEdit ? "✏️ Edit" : "✨ New"}{" "}
              {form.type === "QUOTATION" ? "Quotation" : "Invoice"}
            </h1>
            <p className="form-page-sub">
              {form.invoiceNumber
                ? form.invoiceNumber
                : "Enter number in Invoice Info tab →"}
            </p>
          </div>
        </div>
